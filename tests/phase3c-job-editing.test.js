...
test("admin/booking.js write-audit stays exact: this stage's one new .update( call, plus Stage 2.5-v2's later, separately-reviewed charges writes", () => {
  ...
  // PATCH handleUpdate(). Phase 3C Stage 2.5-v2 (originally built on
  // Braintree, switched to Stripe before any production rollout) later
  // added eleven more, all inside the separate ?resource=charges
  // propose/approve/check-status workflow: approved-transition,
  // processing-transition, the initial paid-attempt, markChargeFailed,
  // markChargeErrorPendingReview, markChargeRequiresCustomerAction
  // (Stripe-specific), the retryUpdate-wrapped full paid-confirmation
  // retry plus its minimal paid_reconciliation_required fallback write,
  // and handleCheckStatus's two conditional reconciliation writes — see
  // tests/phase3c-schedule.test.js's own write-audit test, and
  // tests/phase3c-stage2.5v2-stripe-rental-payments.test.js, for that
  ...
  // same reasoning again; see that same test file's Tip test. Batch 2
