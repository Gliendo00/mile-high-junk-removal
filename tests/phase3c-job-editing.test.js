...
test("admin/booking.js write-audit stays exact: this stage's one new .update( call, plus Stage 2.5-v2's later, separately-reviewed charges writes", () => {
  ...
  // PATCH handleUpdate(). Phase 3C Stage 2.5-v2 later added five more,
  // all inside the separate ?resource=charges propose/approve workflow
  // (approved-transition, processing-transition, paid-transition,
  // markChargeFailed, and markChargeErrorPendingReview, added in the
  // 2026-09-18 pre-deployment hardening audit — see
  ...
  // this file runs.
  assert.strictEqual(updateCalls, 6, "booking.js must have exactly 6 .update( calls (1 Edit Job + 5 Stage 2.5-v2 charges) — anything else needs deliberate review");
});
