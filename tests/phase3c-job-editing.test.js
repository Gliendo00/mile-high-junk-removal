...
test("admin/booking.js write-audit stays exact: this stage's one new .update( call, plus Stage 2.5-v2's later, separately-reviewed charges writes", () => {
  ...
  // PATCH handleUpdate(). Phase 3C Stage 2.5-v2 later added seven more, all
  // inside the separate ?resource=charges propose/approve workflow:
  // approved-transition, processing-transition, the initial paid-attempt,
  // markChargeFailed, markChargeErrorPendingReview (hardening audit), and
  // the readiness pass's retryUpdate-wrapped full paid-confirmation retry
  // plus its minimal paid_reconciliation_required fallback write — see
  ...
  // this file runs.
  assert.strictEqual(updateCalls, 8, "booking.js must have exactly 8 .update( calls (1 Edit Job + 7 Stage 2.5-v2 charges) — anything else needs deliberate review");
});
